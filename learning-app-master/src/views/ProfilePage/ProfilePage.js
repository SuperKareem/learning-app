...
export default function ProfilePage(props) {
  ...
  const extractData = React.useCallback(
    (...args) => {
      ...
      let _checked = 0;
      args.forEach(arg =>
        arg.docs.forEach(element => {
          const el = { id: element.id, ...element.data() };
          arr.push(el);
          _barLength = _barLength + el.materials.length;
        })
      );
      return [arr, _barLength, _checked];
    },
    [completedMaterials]
  );
  ...
  const handleMaterialCheck = React.useCallback(
    async (courseIndex, materialIndex) => {
      ...
      const checked = completedMaterials.indexOf(name);
      ...
      if (checked < 0) {
        ...
        materialsCompleted = [...completedMaterials];
        materialsCompleted.splice(checked, 1);
        console.log("ssss", materialsCompleted);
      }
